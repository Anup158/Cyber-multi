...
export default function PDF() {
  const [report, setReport] = useState<{
    name: string;
    size: number;
    hits: { token: string; count: number }[];
  } | null>(null);
    ...
    ? Math.min(
        ...
        report.hits.reduce(
          (s, h) =>
            s +
            (h.count > 0
              ? h.token === "/JavaScript" || h.token === "/Launch"
                ? 35
                : 15
              : 0),
          0,
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <div>
          ...
          <p className="text-sm text-foreground/70">
            Static inspection for risky objects and actions. Upload a PDF to
            analyze.
          </p>
        </div>
        <Card className="border-border/60">
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              />
              <Button variant="secondary" onClick={scanUrl}>
                Scan URL
              </Button>
            </div>
            {report && (
              <div className="space-y-3">
                <div
                  className={`rounded-md border p-3 text-sm ${risk === "high" ? "border-red-500/30 bg-red-500/5 text-red-400" : risk === "medium" ? "border-amber-500/30 bg-amber-500/5 text-amber-400" : "border-emerald-500/30 bg-emerald-500/5 text-emerald-400"}`}
                >
                  Risk: {risk} (score {score}) — {report.name} (
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {report.hits.map((h) => (
                    <div
                      key={h.token}
                      className={`rounded-md border px-3 py-2 text-sm ${h.count > 0 ? "border-red-500/30 bg-red-500/5 text-red-400" : "border-emerald-500/30 bg-emerald-500/5 text-emerald-400"}`}
                    >
                ...
                <p className="text-xs text-foreground/60">
                  Heuristic-only, does not execute embedded code. For full
                  analysis, integrate a sandbox.
                </p>
              </div>
            )}
          </CardContent>
